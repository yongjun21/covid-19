...
export default {
  mounted () {
    ...
    map.on('load', () => {
      ...
      dataPending.then(([provinces, cities]) => {
        const toPlot = []
        provinces.forEach(row => {
          if (!row.name) return
          if (['北京市', '天津市', '上海市', '重庆市'].includes(row.provinceName)) toPlot.push(row)
        })
        cities.forEach(row => {
          if (!row.name) return
          if (!['北京市', '天津市', '上海市', '重庆市'].includes(row.provinceName)) toPlot.push(row)
      ...
      map.on('mousemove', e => {
        const features = map.queryRenderedFeatures(e.point, {
          layers: ['l1_data', 'l2_data']
        })
        if (features.length > 0) {
          tooltip.setData(features[0].properties.name_chi).trackPointer().addTo(map)
        } else {
...
function getData () {
  ...
  return Promise.all([
    ...
    provinces.forEach(row => {
      row.name = provinceNames[row.provinceName]
    })
    cities.forEach(row => {
      ...
      if (!matched) {
        ...
        if (correction) matched = children.find(c => c.name_chi === correction.correctName)
      }
      row.name = matched
    })
